...
public class CoreSingleDataSource<T: NSManagedObject & DataObject>: NSObject, SingleDataSource {
    ...
    @objc private func objectsDidChange(_ notification: Notification) {
        guard let context = notification.object as? NSManagedObjectContext, context === dataAccess.mainContext as? NSManagedObjectContext else { return }
        let allObjects = NSManagedObjectContext.objectsFrom(notification: notification, ofType: T.self)
        if allObjects.refreshes.count == 0, allObjects.inserts.count == 0, allObjects.updates.count == 0, allObjects.deletes.count == 0 {
            return
        }
        
        if let _ = (allObjects.deletes as NSSet).filtered(using: NSCompoundPredicate.init(andPredicateFrom: attributes)).first as? T {
            object = nil
            return
        }
        
        let theRest = allObjects.inserts.union(allObjects.updates).union(allObjects.refreshes)
        if let filtered = (theRest as NSSet).filtered(using: NSCompoundPredicate.init(andPredicateFrom: attributes)).first as? T {
            object = filtered
        }
