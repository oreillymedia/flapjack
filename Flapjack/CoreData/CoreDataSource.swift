...
public class CoreDataSource<T: NSManagedObject & DataObject>: NSObject, NSFetchedResultsControllerDelegate, DataSource {
    ...
    public var numberOfObjects: Int {
        if let limit = limit { return limit }
        return controller.fetchedObjects?.count ?? 0
    }
    
    public var allObjects: [T] {
        guard let fetched = controller.fetchedObjects else { return [] }
        guard let limit = limit else { return fetched }
    ...
    private class func cacheName(for request: NSFetchRequest<T>?) -> String {
        guard let request = request else { return "all" }
        var elements = [String]()
    ...
    public func controllerWillChangeContent(_ controller: NSFetchedResultsController<NSFetchRequestResult>) {
        guard controller === controller else { return }
    ...
    public func controllerDidChangeContent(_ controller: NSFetchedResultsController<NSFetchRequestResult>) {
        guard controller === controller else { return }
        guard let onChange = onChange, !pendingItemChanges.isEmpty || !pendingSectionChanges.isEmpty else { return }
